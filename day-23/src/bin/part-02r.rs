    ...
    let (input, grid) = many1(delimited(
        ...
        alt((
            tag("^").map(|span| {
                ...
                Plot {
                    r#type: PlotType::Directional(Direction::North),
                    span: located,
                }
            }),
            tag(">").map(|span| {
                let located = with_xy(span);
                Plot {
                    r#type: PlotType::Directional(Direction::East),
                    span: located,
                }
            }),
            tag("v").map(|span| {
                let located = with_xy(span);
                Plot {
                    r#type: PlotType::Directional(Direction::South),
                    span: located,
                }
            }),
            tag("<").map(|span| {
                let located = with_xy(span);
                Plot {
                    r#type: PlotType::Directional(Direction::West),
                    span: located,
                }
            }),
            tag(".").map(|span| {
...
#[allow(dead_code)]
fn print_grid(map: &HashMap<&IVec2, &Plot>, boundaries: &IVec2) {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let (_input, grid) = parse_grid(Span::new(input)).expect("should parse a grid");
    ...
    let start_position = grid.iter().min_by_key(|plot| plot.span.extra.y).unwrap();
    let end_position = grid.iter().max_by_key(|plot| plot.span.extra.y).unwrap();

    // grid_walk

    let mut graph = DiGraph::<&Plot, i32>::new();

    let node_map: HashMap<IVec2, NodeIndex> = grid
        .iter()
        .map(|plot| (plot.span.extra, graph.add_node(plot)))
        .collect();
    ...
    grid.iter()
        .flat_map(|plot| {
            ...
            .into_iter()
            .filter_map(|dir| {
                let next_pos = dir.step(&plot.span.extra);
                let can_move_here = grid_map.get(&next_pos).is_some();
                can_move_here.then(|| (node_map[&plot.span.extra], node_map[&next_pos], 1))
            })
        })
        .for_each(|(a, b, weight)| {
...
fn main() {
    ...
    let ans = process(input).expect("msg");
