...
fn brick(input: &str) -> IResult<&str, Brick> {
    let (input, (start, end)) = separated_pair(brick_end, complete::char('~'), brick_end)(input)?;
...
pub fn parse_bricks(input: &str) -> IResult<&str, Vec<Brick>> {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let fallen_bricks = sorted_bricks
        .into_iter()
    ...
    let cube_to_id_map = fallen_bricks
        .iter()
        .enumerate()
        .flat_map(|(id, brick)| brick.cubes.iter().map(move |cube| (cube.xyz(), id)))
    ...
    let dissovable = fallen_bricks
        ...
        .filter(|brick| {
            ...
            let bricks_we_support: Vec<usize> = max_cubes
                .iter()
                .filter_map(|cube| cube_to_id_map.get(&IVec3::new(cube.x, cube.y, max_z + 1)))
                .unique()
                .cloned()
                .collect();
                ...
                .filter(|brick_id| {
                    ...
                    min_cubes
                        ...
                        .filter_map(|cube| {
                            ...
                            cube_to_id_map.get(&IVec3::new(cube.x, cube.y, cube.z - 1))
                        })
                        .unique()
